...
export default function LoginPage({ onAuthSuccess }: Props) {
  ...
  const [form,    setForm]    = useState({ email: "", password: "", username: "", fplTeamId: "" });
  ...
  return (
    ...
    <div className="min-h-screen bg-[#37003c] flex flex-col lg:flex-row overflow-hidden relative">
      ...
      {/* ── RIGHT — auth card ── */}
      {/*
        On mobile: full screen, centred, padded, rounded top corners.
        On desktop: fixed 440px right panel.
      */}
      <div className="relative z-10 w-full lg:w-[440px] bg-white flex flex-col justify-center
                      px-6 sm:px-8 lg:px-11 py-10 lg:py-12
                      rounded-t-3xl lg:rounded-none
                      mt-16 lg:mt-0
                      shadow-[0_-8px_40px_rgba(0,0,0,0.3)] lg:shadow-2xl
                      min-h-[calc(100vh-64px)] lg:min-h-screen">

        {/* Mobile logo */}
        <div className="lg:hidden font-display text-[32px] font-bold text-[#37003c] mb-1 text-center">
        ...
        <div className="flex flex-col gap-3.5">
          {tab === "signup" && (
            <div>
              ...
              <input className="fp-input" placeholder="Samuel Kofi" value={form.username} onChange={set("username")} onKeyDown={onKey} />
            </div>
          )}
          <div>
            <label className="block text-[11px] font-bold uppercase tracking-widest text-gray-400 mb-1.5">Email Address</label>
            <input className="fp-input" type="email" placeholder="you@example.com" value={form.email} onChange={set("email")} onKeyDown={onKey} />
          </div>
          <div>
            <label className="block text-[11px] font-bold uppercase tracking-widest text-gray-400 mb-1.5">Password</label>
            <input className="fp-input" type="password" placeholder="••••••••" value={form.password} onChange={set("password")} onKeyDown={onKey} />
          </div>
          {tab === "signup" && (
            <div>
              <label className="block text-[11px] font-bold uppercase tracking-widest text-gray-400 mb-1.5">FPL Team ID</label>
              <input className="fp-input" placeholder="e.g. 4182739" value={form.fplTeamId} onChange={set("fplTeamId")} onKeyDown={onKey} />
        ...
        <button onClick={handleSubmit} disabled={loading}
          className="btn-primary w-full mt-5 text-[15px] py-3.5 disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        <p className="text-center text-[11px] text-gray-300 mt-3">🔒 Secured by Firebase Auth · Payments by Paystack</p>
      </div>
    </div>
  );
}