...
export default function GroupsPage({ user, onNavigate }: Props) {
  ...
  const confirmJoinLeague = () => {
    ...
    setLeagues((prev) =>
      prev.map((l) => l.id === confirmJoin.id ? { ...l, currentPlayers: l.currentPlayers + 1 } : l)
    );
    setConfirmJoin(null);
    // TODO (Joshua): call joinLeague(confirmJoin.id, user.id) and redirect to Paystack
  };
    ...
    <>
      ...
      <div className="relative overflow-hidden text-center py-10 px-5"
        ...
        <div className="relative z-10">
          ...
          <p className="text-white/50 text-sm mb-7">Join a pot, pay your entry, top the leaderboard.</p>
          <div className="flex gap-4 justify-center flex-wrap">
            {[
              { val: `GHS ${totalInPools.toLocaleString()}`, label: "In active pools" },
              { val: `GW 30`,                                 label: "Current gameweek" },
              { val: `${user.walletBalance} GHS`,            label: "Your balance" },
            ].map((s) => (
              <div key={s.label} className="rounded-xl px-5 py-3 text-center"
                style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)" }}>
                <div className="font-display text-2xl font-bold text-[#00ff87]">{s.val}</div>
      ...
      <div className="max-w-4xl mx-auto px-5 py-6 pb-20">
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-display text-2xl font-bold text-[#37003c]">Active Pots</h2>
          <button onClick={() => setShowCreate(true)} className="btn-green text-sm py-2.5 px-5">
            + Create Pot
          </button>
        </div>
            ...
            {leagues.map((league) => {
              ...
              return (
                <div key={league.id}
                  ...
                  {/* Card header */}
                  <div className="p-4 pb-3" style={{ background: "linear-gradient(135deg, #37003c, #55006a)" }}>
                    <div className="flex items-center gap-3 mb-3">
                      <div className="w-11 h-11 rounded-xl flex items-center justify-center font-display text-xl font-bold text-[#37003c] flex-shrink-0"
                        style={{ background: "#00ff87" }}>
                        {league.name[0]}
                      </div>
                      <div>
                        <p className="text-white font-bold text-[15px] leading-tight">{league.name}</p>
                        <p className="text-white/40 text-[11px]">Gameweek {league.gameweek}</p>
                      </div>
                      {league.status === "active" && (
                        <span className="ml-auto text-[10px] font-bold text-[#00ff87] bg-[#00ff87]/10 border border-[#00ff87]/20 rounded-full px-2.5 py-0.5">
                          LIVE
                        </span>
                      )}
                    </div>
                    ...
                    <div className="h-1 bg-white/10 rounded-full overflow-hidden">
                      <div className="h-full bg-[#00ff87] rounded-full transition-all duration-500"
                        style={{ width: `${pct}%` }} />
                    </div>
                    <div className="flex justify-between text-[10px] text-white/30 mt-1">
                      <span>{league.currentPlayers}/{league.maxPlayers} managers</span>
                      <span>{full ? "Full" : `${league.maxPlayers - league.currentPlayers} spots left`}</span>
                    </div>
                  </div>
                  ...
                  <div className="px-4 pt-3 pb-2">
                    <div className="flex divide-x divide-gray-100 mb-3">
                      <div className="flex-1 pr-4">
                        ...
                        <p className="font-display text-xl font-bold text-[#37003c]">GHS {league.entryFee}</p>
                      </div>
                      <div className="flex-1 pl-4">
                        <p className="text-[9px] uppercase tracking-widest font-bold text-gray-400">Prize Pool</p>
                        <p className="font-display text-xl font-bold text-green-600">GHS {pool}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center mb-3">
                      ...
                      {league.currentPlayers > 5 && (
                        <div className="w-7 h-7 rounded-full border-2 border-white bg-gray-200 text-gray-500 flex items-center justify-center text-[9px] font-bold flex-shrink-0 -ml-2">
                          +{league.currentPlayers - 5}
                        </div>
                      )}
        ...
        <div className="modal-overlay" onClick={() => setShowCreate(false)}>
          <div className="modal-sheet" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-3 text-[13px] text-green-700 mb-5">
              💡 Your wallet will be charged <strong>GHS {newLeague.entryFee}</strong> when the pot is full and starts.
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleCreate} className="flex-[2] btn-primary text-[15px]">
                CREATE POT
              </button>
            </div>
          </div>
        ...
        <div className="modal-overlay" onClick={() => setConfirmJoin(null)}>
          <div className="modal-sheet" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={confirmJoinLeague} className="flex-[2] btn-primary text-[15px]">
                CONFIRM & PAY
              </button>
            </div>
          </div>
      ...
      {showWallet && (
        <WalletModal
          ...
          onDeposit={() => {/* TODO (Joshua): initiateDeposit(user.id, amount) */}}
